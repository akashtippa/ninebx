...
internal class InterestItemsAdapter(private var myList: ArrayList<DecryptedInterests>?, private val mainContactsAdded: MainContactsAdded) : RecyclerView.Adapter<InterestItemsAdapter.RecyclerItemViewHolder>() {
    ...
    fun deleteContact(contact: DecryptedInterests) {
    ...
    fun notifyData(myList: ArrayList<DecryptedInterests>) {
        this.myList = myList
        notifyDataSetChanged()
    }

    fun getList(): ArrayList<DecryptedInterests>? {
        return this.myList
    }

    fun updateContact(contact: DecryptedInterests) {
        for(item in myList!!) {
